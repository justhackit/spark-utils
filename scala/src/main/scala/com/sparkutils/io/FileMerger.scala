...
object FileMerger {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val arrRow = spark.sql("").take(10)
    spark.createDataFrame(spark.sparkContext.parallelize(arrRow),arrRow(0).schema).coalesce(1).write.mode("append")
    ...
    val missedOnes = List("app_domain",
      "app_name",
      "card_id",
      "cta_deep_link",
      "currency",
      ...
      "event_type",
      "experience_channel",
      "integrations_adobe_analytics_marketing_cloud_visitor_id",
      "landmarkx",
      "landmarky",
      "launch_type",
      "login_status",
      ...
      "order_id",
      "os_name",
      "payments_concatenated",
      "previous_view_pagetype",
      "process_date",
      "product_id",
      "search_term_type",
      "session_start_ts",
      ...
      "timestamp",
      "user_agent")
    missedOnes.foreach(attrib => {
      spark.read.parquet(s"s3://nikebi-managed/dev/ck/consumer360/AJ/results/$attrib/notnull/").show(10,false)
      spark.read.parquet(s"s3://nikebi-managed/dev/ck/consumer360/AJ/results/$attrib/dist/").show(10,false)
      println("====================================")
    })
